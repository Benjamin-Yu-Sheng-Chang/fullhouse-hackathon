"""
STRATEGY OVERVIEW
Abstract Control v2 hybridizes Abstract Control v1's board-texture discipline
with Adaptive Hybrid v2's stack/profile awareness. It keeps pots smaller on
dangerous multiway boards, value-bets strong abstractions more confidently,
uses short-stack rules only preflop/early, and applies late-position pressure
only in low-risk heads-up or low-aggression spots.
"""
import random
...
def _preflop_policy(state, features):
    ...
    if premium:
        multiplier = 3.4 if profile["label"] == "aggressive" else 2.8
...
def _postflop_policy(state, features):
    ...
    if texture["made"] == "strong" and owed <= state["your_stack"] * 0.40:
        return {"action": "raise", "amount": _bet_to(state, CONFIG["raise_bet_fraction"] if not texture["board_danger"] else 0.62)}
