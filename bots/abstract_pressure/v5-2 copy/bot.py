"""Abstract Pressure v5-2: v5 pressure balanced with Chen score and opponent reads."""
import random
import time
...
BOT_NAME = "Abstract Pressure v5-2"
BIG_BLIND = 100
RANKS = "23456789TJQKA"
RANK_VALUE = {rank: value for value, rank in enumerate(RANKS, 2)}

# Wall-clock budget for postflop Monte Carlo (sandbox cap is 2 s/decision).
BUDGET_S = 0.25
POSTFLOP_ITERS = 8000
_RNG = random.Random()
...
CONFIG = {
    ...
    "raise_equity": 0.68,
    "call_margin": 0.058,
    "multiway_margin": 0.044,
    "semi_bluff_frequency": 0.32,
    "thin_value_pair": True,
    "pressure_frequency": 0.28,
    "river_extra_margin": 0.055,
    "large_bet_extra_margin": 0.095,
    "danger_extra_margin": 0.025,
    "model_min_actions": 16,
}

CHEN_HIGH_CARD = {14: 10.0, 13: 8.0, 12: 7.0, 11: 6.0}
for value in range(2, 11):
    CHEN_HIGH_CARD[value] = value / 2.0
...
def _fold_equity(state, model):
    ...
    for bot_id in ids:
        ...
        if profile["archetype"] == "station":
            fold_equity = min(fold_equity, 0.05)
        elif profile["archetype"] == "maniac":
            fold_equity = min(fold_equity, 0.15)
        elif profile["archetype"] == "unknown":
...
def _steal_loosen(state, model):
    ...
    return max(0.0, min(1.0, (fold_freq - 0.45) * 3.0))
...
def _chen_score(cards):
    high, low, suited, pair, gap_raw = _preflop_features(cards)
    if pair:
        return max(CHEN_HIGH_CARD[high] * 2.0, 5.0)
    score = CHEN_HIGH_CARD[high]
    if suited:
        score += 2.0
    gap = high - low - 1
    if gap == 1:
...
_TEXTURE_EQUITY = {"strong": 0.78, "medium": 0.55, "weak_pair": 0.42, "air": 0.32}


def _equity(state, opponents, iters, deadline):
    """Hero equity in [0, 1] vs random opponents; stops at ``deadline``."""
    ...
    if draw > len(deck) or opponents <= 0:
        return 0.5, 0
    ...
    for i in range(iters):
        ...
        runout = drawn[:need]
        idx = need
        hero = evaluate(hole + board + runout)
        tied = lost = False
        for _ in range(opponents):
            villain = evaluate([drawn[idx], drawn[idx + 1]] + board + runout)
            idx += 2
            if villain > hero:
                lost = True
                break
    ...
    if not completed:
        return 0.5, 0
    return (wins + 0.5 * ties) / completed, completed


def _resolve_equity(equity, completed, texture):
    """Blend MC estimate with texture prior when sample count is low."""
    if completed >= 8:
        return equity
    prior = _TEXTURE_EQUITY.get(texture["made"], 0.5)
    if completed == 0:
        return prior
    weight = completed / 8.0
    return weight * equity + (1.0 - weight) * prior


def _bet_to(state, fraction, allow_all_in=False):
    cap = state["your_stack"] + state["your_bet_this_street"]
    amount = max(state["min_raise_to"], int(max(BIG_BLIND, state["pot"]) * fraction))
    if not allow_all_in and amount >= cap and cap > state["min_raise_to"]:
        amount = max(state["min_raise_to"], int(cap * 0.72))
    return min(amount, cap)


def _raise_to(state, amount):
    cap = state["your_stack"] + state["your_bet_this_street"]
    if amount >= cap and cap > state["min_raise_to"]:
        amount = max(state["min_raise_to"], int(cap * 0.72))
    return min(amount, cap)
...
def _preflop(state, features):
    ...
    if profile["label"] == "foldy" and pos == "late":
        open_line -= 0.5
    if pos == "late":
        open_line -= min(0.6, _steal_loosen(state, model))
    if opponents >= 5:
        open_line += 0.4

    if premium:
        return {"action": "raise", "amount": _raise_to(state, max(state["min_raise_to"], int(max(pot, state["current_bet"]) * 1.2)))}
    ...
    if owed and score >= continue_line and owed <= pot * 0.30:
        return {"action": "call"}
    if owed and score >= continue_line - 1.0 and owed <= pot * 0.14:
        return {"action": "call"}
    if owed and speculative and owed <= pot * 0.18:
        return {"action": "call"}
    if owed and broadwayish and pressure_score >= 47 and owed <= pot * 0.16:
        return {"action": "call"}
    if state["can_check"]:
        return {"action": "check"}
    return {"action": "fold"}
...
def _postflop(state, features):
    ...
    if owed and owed <= pot * 0.16 and state["street"] != "river":
        margin -= 0.012

    if owed == 0:
        value_line = CONFIG["value_equity"]
        value_size = 0.75 if not texture["board_danger"] else 0.58
        if station_field:
            value_line -= 0.035
            value_size = 0.50
        elif loose_field:
        ...
        if texture["made"] == "strong" or equity >= value_line:
            return {"action": "raise", "amount": _bet_to(state, value_size, texture["made"] == "strong" or equity >= 0.80)}
        if CONFIG["thin_value_pair"] and pos == "late" and texture["made"] == "medium" and opponents <= 2 and equity >= value_line - 0.05:
        ...
        if loose_field:
            semi_bluff_frequency = 0.08
        elif tight_field:
            semi_bluff_frequency = min(0.40, semi_bluff_frequency + 0.06)
        if pos == "late" and opponents <= 2 and (texture["flush_draw"] or texture["straight_draw"]) and fold_equity * pot > (1 - fold_equity) * max(1, int(pot * 0.52)) and _RNG.random() < semi_bluff_frequency:
            return {"action": "raise", "amount": _bet_to(state, 0.52)}
        pressure_frequency = CONFIG["pressure_frequency"]
        if loose_field:
            pressure_frequency = 0.06
        elif tight_field:
            pressure_frequency = min(0.36, pressure_frequency + 0.06)
        if pos == "late" and profile["label"] == "foldy" and equity > 0.40 and fold_equity * pot > (1 - fold_equity) * max(1, int(pot * 0.42)) and _RNG.random() < pressure_frequency:
            return {"action": "raise", "amount": _bet_to(state, 0.42)}
        return {"action": "check"}

    if texture["made"] == "strong" and owed <= state["your_stack"] * 0.45:
        return {"action": "raise", "amount": _bet_to(state, 0.95, True)}
    ...
    if equity >= raise_line and owed <= state["your_stack"] * 0.35:
        return {"action": "raise", "amount": _bet_to(state, 0.78, equity >= 0.82)}
    draw_raise_frequency = CONFIG["semi_bluff_frequency"]
    if loose_field:
        draw_raise_frequency = 0.08
    elif tight_field:
        draw_raise_frequency = min(0.38, draw_raise_frequency + 0.05)
    if (texture["flush_draw"] or texture["straight_draw"]) and pos == "late" and owed <= pot * 0.20 and fold_equity * pot > (1 - fold_equity) * max(1, owed) and _RNG.random() < draw_raise_frequency:
        return {"action": "raise", "amount": _bet_to(state, 0.58)}
    if owed <= pot * 0.14 and state["street"] != "river" and texture["made"] in ("weak_pair", "medium"):
        return {"action": "call"}
    if owed <= pot * 0.12 and state["street"] != "river" and (texture["flush_draw"] or texture["straight_draw"]):
        return {"action": "call"}
    if owed > state["your_stack"] * 0.34 and texture["made"] != "strong" and equity < 0.72:
        return {"action": "fold"}
    if state["street"] == "river" and owed > pot * 0.42 and texture["made"] != "strong" and equity < 0.66:
        return {"action": "fold"}
...
def _decide(state):
    ...
    features = {
        ...
        "chen": _chen_score(state["your_cards"]),
    ...
    raw_eq, completed = _equity(state, opponents, POSTFLOP_ITERS, deadline)
    features["equity"] = _resolve_equity(raw_eq, completed, features["texture"])
