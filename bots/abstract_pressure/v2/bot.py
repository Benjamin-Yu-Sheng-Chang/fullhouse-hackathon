"""
Abstract Pressure v2: pressure's steal/semi-bluff engine with adaptive guardrails.

This version keeps v1's chip-delta hunting identity, but borrows adaptive_hybrid
style stack mode, SPR, table profile tightening, and short-stack decisions so
the bot avoids some low-equity pressure spots that bust too often.
"""
import random
...
CONFIG = {
    ...
    "aggro_tighten": 5,
    "short_stack_bb": 18,
    "short_stack_jam_score": 74,
    "short_stack_call_score": 72,
    "pressure_spr_floor": 1.9,
    "sticky_value_bonus": 0.08,
    "late_air_pressure_frequency": 0.10,
}
...
def _stack_mode(state):
    ...
    if bb <= CONFIG["short_stack_bb"]:
        ...
        mode = "normal"
    return {"bb": bb, "mode": mode, "spr": state["your_stack"] / max(1, state["pot"])}


def _pot_price(state):
    owed = state["amount_owed"]
    pot = max(1, state["pot"])
    return owed / (pot + owed) if owed else 0.0
...
def _short_stack_policy(state, features):
    ...
    high, low, suited, pair, _gap = features["hand"]
    ...
    if owed and score >= CONFIG["short_stack_call_score"] and owed <= state["your_stack"] * 0.34:
        return {"action": "call"}
    if state["can_check"]:
        return {"action": "check"}
    return {"action": "fold"}
...
def _preflop(state, features):
    ...
    if _very_late(state) and profile["label"] != "aggressive" and opponents <= 3:
        open_line -= 4
    if opponents >= 5:
        open_line += 3

    if premium:
        multiplier = 3.8 if profile["label"] == "aggressive" else 3.1 if profile["label"] == "sticky" else 2.8
        size = max(state["min_raise_to"], int(max(state["current_bet"], BIG_BLIND) * multiplier), int(pot * 0.95))
...
def _postflop(state, features):
    ...
    if owed == 0:
        if texture["made"] == "strong" or equity >= CONFIG["value_equity"]:
            fraction = 0.75 if not texture["board_danger"] else 0.58
        ...
        if CONFIG["thin_value_pair"] and pos == "late" and texture["made"] == "medium" and opponents <= 2 and features["street_aggression"] == 0:
            return {"action": "raise", "amount": _bet_to(state, 0.45)}
        can_pressure = pos == "late" and opponents <= 2 and features["street_aggression"] == 0 and stack["spr"] >= CONFIG["pressure_spr_floor"]
        if can_pressure and (texture["flush_draw"] or texture["straight_draw"]) and equity >= 0.38 and random.random() < CONFIG["semi_bluff_frequency"]:
            return {"action": "raise", "amount": _bet_to(state, 0.52)}
        if can_pressure and profile["label"] == "foldy" and equity > 0.42 and random.random() < CONFIG["pressure_frequency"]:
            return {"action": "raise", "amount": _bet_to(state, 0.42)}
        if can_pressure and profile["label"] == "foldy" and opponents == 1 and equity >= 0.35 and random.random() < CONFIG["late_air_pressure_frequency"]:
            return {"action": "raise", "amount": _bet_to(state, 0.36)}
        return {"action": "check"}

    if texture["made"] == "strong" and owed <= state["your_stack"] * 0.42:
        return {"action": "raise", "amount": _bet_to(state, 0.88 if texture["board_danger"] else 0.95)}
    if equity >= CONFIG["raise_equity"] and owed <= state["your_stack"] * 0.35 and opponents <= 3:
        return {"action": "raise", "amount": _bet_to(state, 0.78)}
    if (texture["flush_draw"] or texture["straight_draw"]) and pos == "late" and opponents <= 2 and owed <= pot * 0.18 and random.random() < CONFIG["semi_bluff_frequency"]:
