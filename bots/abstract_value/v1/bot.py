"""Abstract Value v1: texture-aware value betting with controlled aggression."""
import random
from collections import Counter

import eval7

BOT_NAME = "Abstract Value v1"
BIG_BLIND = 100
RANKS = "23456789TJQKA"
RANK_VALUE = {rank: value for value, rank in enumerate(RANKS, 2)}

CONFIG = {
    "equity_iterations": 90,
    "open_early": 63,
    "open_middle": 57,
    "open_late": 50,
    "continue_early": 69,
    ...
    "raise_equity": 0.72,
    "call_margin": 0.055,
    "multiway_margin": 0.04,
    "semi_bluff_frequency": 0.18,
    "thin_value_pair": True,
    "pressure_frequency": 0.10,
}
...
def _postflop(state, features):
    ...
    if owed == 0:
        if texture["made"] == "strong" or equity >= CONFIG["value_equity"]:
            return {"action": "raise", "amount": _bet_to(state, 0.68 if not texture["board_danger"] else 0.55)}
