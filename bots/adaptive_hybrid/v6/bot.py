...
BOT_NAME = "Adaptive Hybrid v6 GTO Opens"
...
def _gto_open_preflop_action(state):
    if len(state.get("players", [])) != 6 or state.get("street") != "preflop":
        return None
    opener = _gto_first_rfi_position(state)
    if opener is not None:
        if opener == "SB" and _gto_position_from_blinds(state) == "BB" and state["can_check"]:
            spot = GTO_PREFLOP_TABLE.get("vs_limp", {}).get("BB_vs_SB")
            if not spot:
                return None
            action_name = _gto_frequency_action(spot, _gto_hand_code(state["your_cards"]))
            if action_name:
                return _gto_table_action_from_name(state, action_name, spot, "BB")
            return {"action": "check"}
        return None
    if state["current_bet"] > BIG_BLIND:
        return None
...
def decide(state):
    ...
    if state["street"] == "preflop":
        table_preflop_action = _gto_open_preflop_action(state)
